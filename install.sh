...
# @author Adam Eivy
# @modified Joshua Payne
###########################

# include my library helpers for colorized echo and require_brew, etc
source ./lib/sh/echos.sh
source ./lib/sh/required.sh
...
if [[ $response =~ (yes|y|Y) ]];then
    ...
    ok
    bot "Your /etc/hosts file has been updated. Last version is saved in /etc/hosts.bak"
fi
...
for file in .*; do
  ...
  if [[ -e ~/$file ]]; then
      ...
      echo "backup saved as ~/.dotfiles-backup/$now/$file"
  fi
...
running "Set highlight color to teal"
defaults write NSGlobalDomain AppleHighlightColor -string "0.387000 0.821000 1.000000";ok
